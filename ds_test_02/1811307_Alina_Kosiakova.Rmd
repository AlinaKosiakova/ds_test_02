...
```{r}
duomenys <- namq_10_gdp %>%
filter (geo %in% c("LT", "LV", "EE")) %>%
filter (na_item %in% c("D1")) %>%
filter (s_adj %in% c("SCA")) %>%
...
mutate (kumuliatyvi_suma=cumsum(proc.)) %>%
```
...
```{r, fig.width=10}
ggplot (duomenys, aes(time, values, proc, cd = time, group = time))+
  geom_line()+
